...
public:
	...
	void Draw(Shader& shader, int wave_type, float cur_frame)
	{
		if (wave_type == 2) {
			for (unsigned int i = 0; i < meshes.size(); i++) {
				...
				const int DIFFUSE_TEXTURE_INDEX = 0;
				glActiveTexture(GL_TEXTURE0 + DIFFUSE_TEXTURE_INDEX);
				//glUniform1i(glGetUniformLocation(shader.Program, "height_map_texture"), height_maps[cur_height_map_index]);
				glBindTexture(GL_TEXTURE_2D, height_maps[cur_height_map_index]);
				cur_mesh->Draw(shader);
				//cout << "draw height_map " << height_maps[cur_height_map_index] << endl;
			}
	...
	void add_height_map_textures(const string& _directory) {
		for (int i = 0; i < 200; i++) {
